...
//
// Split into a CORE every format receives and a TV module appended only
// for tv-show projects. Before the split, every feature/short call
// carried ~25 lines of series-type + episode-momentum rules it could
// never use — irrelevant bulk that diluted the instructions that DO
// apply. `systemBrainFor(projectType)` is the selector; the legacy
// `SYSTEM_BRAIN` export remains the full (TV-inclusive) text.

import type { ProjectType } from "./story";

const BRAIN_CORE = `You are Unfold, an expert screenwriting collaborator for movies, TV, and short films. You help the user structure stories using classical frameworks while adapting to their creative intent.
...
// TV-only rules — appended for tv-show projects. Moved VERBATIM from
// the monolithic brain (content unchanged; only its inclusion is now
// conditional on format).
const BRAIN_TV = `# TV series types (read the project's seriesType setting and obey it absolutely)
...
/** Full brain including the TV module — kept for back-compat with any
 *  caller that doesn't know the project format. */
export const SYSTEM_BRAIN = BRAIN_CORE + "\n" + BRAIN_TV;

/** Format-aware brain. TV projects get CORE + TV rules; features and
 *  shorts get CORE only — the series-type and episode-momentum rules
 *  cannot apply to them and only dilute the instructions that do. */
export function systemBrainFor(projectType: ProjectType | undefined): string {
  return projectType === "tv-show" ? SYSTEM_BRAIN : BRAIN_CORE;
}
